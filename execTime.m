...
for AnnId = 4:size(AnInfo,1)
    %Load Image Frames
    [Mth1Time, Mth2Time, Mth3Time, Mth4Time, Mth5Time, Mth6Time, Mth7Time, Mth8Time, Mth9Time, Mth10Time] = deal(0);
    FolderDir ="E:";
    ...
    % hWaitBar = waitbar(0, 'Reading the CT Images');
        ...
        for f = LagLead : incr : img_NUM-LagLead
            ...
            Mth1Time = Mth1Time + toc(startTime);
...
%             
            ...
            Mth2Time = Mth2Time + toc(startTime);
            %% -U02- Implement Cosfire Method for Evaluation              
            startTime = tic;    SegTime = 0;
            ...
            Mth3Time = Mth3Time + toc(startTime);
...
%             %saveas(gcf, [DataDir, 'Evaluation\Seq' num2str(AnnId),'-Frm', num2str(f), '-Col10.png'])
            ...
            Mth6Time = Mth6Time + toc(startTime);
...
%             saveas(gcf, [DataDir, 'Li results\Seq' num2str(AnnId),'-Frm', num2str(f), '-Col12.png'])
    ...
    compTime(AnnId,:) = [Mth1Time, Mth2Time, Mth3Time, Mth4Time, Mth5Time, Mth6Time, Mth7Time, Mth8Time, Mth9Time, Mth10Time]/tracker-1;
